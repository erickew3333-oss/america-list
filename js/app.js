...
function viewOrder(id){window._currentView='viewOrder';window._currentOrderId=id;const o=db.orders.find(x=>x.id===id);if(!o)return;const canEdit=db.user?.role==='admin';app.innerHTML=`${header()}<main class="wrap">${nav()}<section class="card"><div class="row"><button type="button" class="btn gray" onclick="orders()">← Voltar às O.S.</button><h2 style="margin-right:auto">Protocolo ${esc(o.protocol)}</h2><span class="pill">${esc(o.status)}</span></div><div class="notice"><b>Data e hora da O.S.:</b> ${esc(o.created)}${o.updated?`<br><span class="small">Última alteração: ${esc(o.updated)}</span>`:''}</div><div class="grid"><div><b>Motorista</b><p>${esc(o.driver)}</p></div><div><b>Placa</b><p>${esc(o.plate)}</p></div><div><b>Associado</b><p>${esc(o.assoc)}</p></div><div><b>Tipo de Veículo</b><p>${esc(o.vehicleType)}</p></div><div><b>Modelo</b><p>${esc(o.model)}</p></div><div><b>Cor</b><p>${esc(o.color)}</p></div><div><b>Protocolo</b><p>${esc(o.protocol)}</p></div><div><b>Local Origem</b><p>${esc(o.origin)||'-'}</p></div><div><b>Local Destino</b><p>${esc(o.destination)||'-'}</p></div><div><b>KM</b><p>${esc(o.km)||'-'}</p></div><div><b>Pneus</b><p>${esc(o.tires)||'-'}</p></div><div><b>Chaves Acompanhando</b><p>${esc(o.keys)||'-'}</p></div></div><h3>Danos ou Avarias Pré-Existentes</h3><p>${o.damages?.length?o.damages.map(esc).join(', '):'Nenhuma informada.'}</p><h3>Checklist</h3>${checklist.map(k=>`<div class="check"><span>${k}</span><b>${esc(o.checks[k])}</b></div>`).join('')}<h3>Observações</h3><p>${esc(o.obs)||'-'}</p><h3>Assinatura do Associado</h3>${o.signatureData?`<div class="signature-preview"><img src="${esc(o.signatureData)}" alt="Assinatura do associado"></div>${o.signatureLat!=null&&o.signatureLng!=null?`<div class="signature-location"><b>📍 Local da assinatura:</b> ${Number(o.signatureLat).toFixed(6)}, ${Number(o.signatureLng).toFixed(6)}${o.signatureAccuracy?` · precisão aproximada ${Math.round(o.signatureAccuracy)} m`:''}<br><a href="https://www.google.com/maps?q=${o.signatureLat},${o.signatureLng}" target="_blank" rel="noopener">Abrir localização no mapa</a></div>`:''}`:'<span class="small">Sem assinatura registrada.</span>'}<h3>Fotos</h3>${o.photos?.map(p=>`<a href="${p}" download="foto-os-${o.protocol}.jpg" title="Baixar foto"><img class="photo" src="${p}"></a>`).join('')||'<span class="small">Sem fotos.</span>'}<div class="row" style="margin-top:18px">${canEdit?`<button class="btn" onclick="editOrder('${o.id}')">Editar O.S.</button>`:''}<button class="btn" onclick="printOrder('${o.id}')">Salvar / Imprimir PDF</button><button class="btn green" onclick="finishOrder('${o.id}')">Finalizar O.S.</button><button class="btn red" onclick="problemOrder('${o.id}')">Marcar com problema</button><button class="btn gray" onclick="orders()">Voltar</button></div></section></main>`}
...
function printOrder(id){
 ...
 <section class="section"><h3>Assinatura do Associado</h3>${o.signatureData?`<div><img src="${esc(o.signatureData)}" style="max-width:320px;max-height:110px;object-fit:contain;border-bottom:1px solid #9eabb8"></div>${o.signatureLat!=null&&o.signatureLng!=null?`<div style="margin-top:8px;padding:8px;border:1px solid #dce3eb;border-radius:5px"><b>Local da assinatura:</b> ${Number(o.signatureLat).toFixed(6)}, ${Number(o.signatureLng).toFixed(6)}${o.signatureAccuracy?` · precisão aproximada ${Math.round(o.signatureAccuracy)} m`:''}<br><a href="https://www.google.com/maps?q=${o.signatureLat},${o.signatureLng}" target="_blank" rel="noopener">Abrir localização no mapa</a></div>`:''}`:'<div class="empty">Sem assinatura registrada</div>'}</section><section class="section"><h3>Fotos do atendimento</h3><div class="photos">${photos}</div></section>
...
function manageUsers(){window._currentView='users';
...
function requestSignatureLocation(){
  if(!navigator.geolocation){
    const s=document.getElementById('signatureLocationStatus'); if(s)s.textContent='GPS não disponível neste dispositivo.';
    return Promise.resolve(null);
  }
  const s=document.getElementById('signatureLocationStatus');
  if(s)s.textContent='Obtendo localização do dispositivo…';
  return new Promise(resolve=>{
    navigator.geolocation.getCurrentPosition(pos=>{
      signatureLocation={latitude:pos.coords.latitude,longitude:pos.coords.longitude,accuracy:pos.coords.accuracy,capturedAt:new Date().toISOString()};
      if(s)s.textContent=`Localização capturada (${signatureLocation.latitude.toFixed(6)}, ${signatureLocation.longitude.toFixed(6)})`;
      resolve(signatureLocation);
    },err=>{
      signatureLocation=null;
      if(s)s.textContent=err.code===1?'Permissão de localização negada. A assinatura continuará sem GPS.':'Não foi possível obter o GPS. A assinatura continuará sem localização.';
      resolve(null);
    },{enableHighAccuracy:true,timeout:7000,maximumAge:30000});
  });
}
window.captureSignatureLocation=()=>requestSignatureLocation();
function initSignaturePad(){
 ...
 canvas.addEventListener('pointerdown',e=>{e.preventDefault();drawing=true;last=point(e);canvas.setPointerCapture?.(e.pointerId);if(!signatureLocation)requestSignatureLocation()});
...
window.clearSignature=()=>{const c=document.getElementById('signaturePad');if(!c)return;const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);c.dataset.signed='0';signatureLocation=null;const s=document.getElementById('signatureLocationStatus');if(s)s.textContent='Aguardando assinatura e localização.';};
window.saveOrder=async()=>{
 ...
 if(signatureData && !signatureLocation) await requestSignatureLocation();
 const payload={driver_id:driver.id,associated_name:document.getElementById('assoc').value.trim(),plate:document.getElementById('plate').value.trim().toUpperCase(),vehicle_type:document.getElementById('vehicleType').value,vehicle_model:document.getElementById('model').value.trim(),vehicle_color:document.getElementById('color').value,protocol,origin:document.getElementById('origin').value.trim()||null,destination:document.getElementById('destination').value.trim()||null,mileage:document.getElementById('km').value===''?null:Number(document.getElementById('km').value),preexisting_damages:damages,tires:document.getElementById('tires').value||null,keys_with:document.getElementById('keys').value||null,checklist:{items:checks,observations:document.getElementById('obs').value||''},status:'Em atendimento',created_by:currentAuthUser.id,signature_data:signatureData||null,signed_at:signedAt,signature_location_latitude:signatureLocation?.latitude??null,signature_location_longitude:signatureLocation?.longitude??null,signature_location_accuracy:signatureLocation?.accuracy??null,signature_location_captured_at:signatureLocation?.capturedAt??null};
...
window.deleteOrder=async id=>{if(db.user?.role!=='admin')return;const o=db.orders.find(x=>x.id===id);if(!o)return;openDeleteConfirm([id],`Excluir a O.S. do protocolo ${o.protocol}? Esta ação não pode ser desfeita.`)};

function updateBulkOrderUI(){const bar=document.getElementById('bulkOrderBar');if(!bar)return;const count=selectedOrderIds.size;const c=document.getElementById('selectedOrdersCount');if(c)c.textContent=count;bar.classList.toggle('has-selection',count>0);const btn=bar.querySelector('.btn.red');if(btn)btn.disabled=count===0;const visible=[...(window._ordersBase||[])];const all=visible.length>0&&visible.every(o=>selectedOrderIds.has(o.id));const master=document.querySelector('input[aria-label="Selecionar todas as O.S. visíveis"]');if(master)master.checked=all;}
window.toggleOrderSelection=(id,checked)=>{if(checked)selectedOrderIds.add(id);else selectedOrderIds.delete(id);updateBulkOrderUI()};
window.toggleAllVisibleOrders=checked=>{const visible=[...(window._ordersBase||[])];visible.forEach(o=>checked?selectedOrderIds.add(o.id):selectedOrderIds.delete(o.id));const p=(document.getElementById('qProtocol')?.value||'').toLowerCase(),d=(document.getElementById('qDriver')?.value||'').toLowerCase(),pl=(document.getElementById('qPlate')?.value||'').toLowerCase();const l=visible.slice().reverse().filter(o=>String(o.protocol).toLowerCase().includes(p)&&String(o.driver).toLowerCase().includes(d)&&String(o.plate).toLowerCase().includes(pl));document.getElementById('orderResults').innerHTML=orderTable(l,true);updateBulkOrderUI()};
window.clearOrderSelection=()=>{selectedOrderIds.clear();document.querySelectorAll('[data-order-select]').forEach(el=>el.checked=false);updateBulkOrderUI()};
window.confirmBulkDeleteOrders=()=>{if(db.user?.role!=='admin')return;const ids=[...selectedOrderIds];if(!ids.length)return;const protocols=ids.map(id=>db.orders.find(o=>o.id===id)?.protocol).filter(Boolean);const msg=protocols.length<=5?`Você selecionou ${ids.length} O.S. para exclusão${protocols.length?` (protocolos: ${protocols.join(', ')})`:''}. Deseja realmente excluir essas ordens? Esta ação não pode ser desfeita.`:`Você selecionou ${ids.length} O.S. para exclusão. Deseja realmente excluir todas essas ordens? Esta ação não pode ser desfeita.`;openDeleteConfirm(ids,msg)};
function openDeleteConfirm(ids,message){if(document.getElementById('deleteConfirmModal'))return;document.body.insertAdjacentHTML('beforeend',`<div id="deleteConfirmModal" class="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="deleteConfirmTitle"><div class="confirm-backdrop" onclick="closeDeleteConfirm()"></div><section class="confirm-card"><div class="confirm-icon">!</div><div class="confirm-copy"><span class="section-kicker">CONFIRMAÇÃO DE EXCLUSÃO</span><h2 id="deleteConfirmTitle">Excluir O.S.?</h2><p>${esc(message)}</p></div><div class="confirm-actions"><button type="button" class="btn gray" onclick="closeDeleteConfirm()">Cancelar</button><button type="button" class="btn red" onclick="executeDeleteConfirmed()">Sim, excluir</button></div></section></div>`);window._pendingDeleteIds=ids}
window.closeDeleteConfirm=()=>{document.getElementById('deleteConfirmModal')?.remove();window._pendingDeleteIds=[]};
window.executeDeleteConfirmed=async()=>{const ids=[...(window._pendingDeleteIds||[])];if(!ids.length)return closeDeleteConfirm();const btn=document.querySelector('#deleteConfirmModal .btn.red');if(btn){btn.disabled=true;btn.textContent='Excluindo…'}try{const {error}=await sb.from('service_orders').delete().in('id',ids);if(error)throw error;selectedOrderIds.clear();closeDeleteConfirm();await refreshOnline();if(window._currentView==='orders')orders();else dashboard();alert(`${ids.length===1?'O.S. excluída':'O.S. excluídas'} com sucesso.`)}catch(e){if(btn){btn.disabled=false;btn.textContent='Sim, excluir'}alert(e.message||'Não foi possível excluir as O.S.')} };
  ...
  const refreshAndRender=async(type)=>{
    ...
    try{
      ...
      if(type==='orders'){
        ...
        if(window._currentView==='users') manageUsers();
        else if(window._currentView==='commands') commands();
        else if(window._currentView==='profile') profile();
      } else if(type==='drivers'){
        if(window._currentView==='drivers') manageDrivers();
        else if(window._currentView==='dashboard') dashboard();
      }
...
document.addEventListener('visibilitychange',()=>{
  if(!document.hidden && currentAuthUser){
    refreshOnline().then(()=>{
      if(window._currentView==='dashboard') dashboard();
      else if(window._currentView==='orders') orders();
      else if(window._currentView==='viewOrder' && window._currentOrderId) viewOrder(window._currentOrderId);
...
window.manualRefresh=async()=>{if(!currentAuthUser)return login();const buttons=[...document.querySelectorAll('button[onclick="manualRefresh()"]')];buttons.forEach(b=>{b.disabled=true;b.dataset.oldText=b.textContent;b.textContent='↻ Atualizando…'});try{await refreshOnline();const v=window._currentView;if(v==='dashboard')dashboard();else if(v==='orders')orders();else if(v==='viewOrder'&&window._currentOrderId)viewOrder(window._currentOrderId);else if(v==='users')manageUsers();else if(v==='drivers')manageDrivers();else if(v==='profile')profile();else dashboard();}catch(e){alert(e.message||'Não foi possível atualizar os dados.')}finally{buttons.forEach(b=>{b.disabled=false;b.textContent=b.dataset.oldText||'↻ Atualizar'})}};
...
(async()=>{applyTheme(localStorage.getItem(THEME_KEY)||'light');try{const ok=await initializeOnlineDatabase();if(ok){startRealtime();startSyncFallback();dashboard()}else login();}catch(e){console.error(e);login()}renderThemeButton();})();
