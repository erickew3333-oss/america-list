...
-- Localização opcional da assinatura: preenchida somente quando o dispositivo
-- permitir geolocalização no momento da assinatura.
alter table public.service_orders add column if not exists signature_location_latitude double precision;
alter table public.service_orders add column if not exists signature_location_longitude double precision;
alter table public.service_orders add column if not exists signature_location_accuracy double precision;
alter table public.service_orders add column if not exists signature_location_captured_at timestamptz;
